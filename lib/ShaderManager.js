...
var ShaderManager = /** @class */ (function () {
    ...
    ShaderManager.prototype.loadShaders = function (shaderDefinitionArray) {
        ...
        var shaders = shaderDefinitionArray.shaders;
        var promises = shaders.map(function (item) { return _this.loadShader(item); });
        return Promise.all(promises);
    };
    ShaderManager.prototype.loadShader = function (options) {
        var _this = this;
        var loadVertexShader = fetch(this._shaderDirectory + "/" + options.id + "/vertex_shader.glsl");
        var loadFragmentShader = fetch(this._shaderDirectory + "/" + options.id + "/fragment_shader.glsl");
        var cUniforms = [];
